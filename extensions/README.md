...
Only `pstdio-core-worktree-automation` declares repo scope; the other core extensions use user scope. Subsequent project creates skip existing installs, so user edits under `~/.pstdio-dev/extensions/pstdio-core-*/` and repo-local edits under `<repo>/.pstdio/extensions/` survive across restarts.
...
```ts
{
  defaultExtensions: [
    ...
    { source: "./extensions/pstdio-core-project-repos",          skipInstall: true },
