...
describe("reorderTicketCommand", () => {
  test("moves a ticket before a target ticket and persists sequential sort order", async () => {
    ...
    const first = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "First" } }));
    const second = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "Second" } }));
    const third = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "Third" } }));

    await reorderTicketCommand.run(
      makeCommandContext({ storage, params: { rowId: third.id, beforeRowId: second.id } }),
    );
  ...
  test("appends to the end when no target ticket is supplied", async () => {
    ...
    await reorderTicketCommand.run(makeCommandContext({ storage, params: { rowId: first.id } }));
  ...
  test("ignores archived tickets when calculating visible board order", async () => {
    const storage = createMemoryStorage();
    const first = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "First" } }));
    const archived = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "Archived" } }));
    const third = await createTicketCommand.run(makeCommandContext({ storage, params: { title: "Third" } }));
    await ticketsCollection(storage).put(archived.id, { ...archived, archived: true, sortOrder: 1 });

    await reorderTicketCommand.run(makeCommandContext({ storage, params: { rowId: third.id, beforeRowId: first.id } }));
