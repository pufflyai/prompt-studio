...
describe("reconcileAttemptCommand", () => {
  test("resumes the same implementation session once after a disconnect", async () => {
    ...
    const result = await reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
  ...
  test("reattaches one retry session when persistence fails after launch", async () => {
    ...
    await expect(reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx))).rejects.toThrow(
    ...
    const reconciled = await reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
  ...
  test("releases a failed review claim so the revision can be reviewed again", async () => {
    ...
    const reconciled = await reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
    const restarted = await runReviewCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
  ...
  test("releases the original claim when a replacement review fails", async () => {
    ...
    const retried = await reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
    const reconciled = await reconcileAttemptCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
    const restarted = await runReviewCommand.run(fixture.ctx, commandParamsFor(fixture.ctx));
