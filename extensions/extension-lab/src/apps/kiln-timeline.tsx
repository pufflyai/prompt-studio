...
export const KilnTimeline = (props: { host: ExampleHost }) => {
  const { host } = props;
  const state = useExampleStore(kilnStore);
  ...
  return (
    <Stack h="full" gap="0" bg="bg.panel" overflow="hidden">
      <HStack px="md" h="10" flexShrink={0} borderBottomWidth="1px" borderColor="border.subtle" justify="space-between">
        <HStack gap="sm">
          <ExampleIcon name="Clapperboard" size={14} />
          ...
          {kilnTracks.map((track) => {
            ...
            return (
              <Button
                ...
                onClick={() => host.navigate({ kind: "page", page: kilnPage, resource: kilnResource(object) })}
              >
                <ExampleIcon name={object.icon} color={`${object.tint}.300`} size={13} />
