...
export const LabPage = () => {
  const { t } = useLabHost();
  const { locale } = useLabHostProps();

  return (
    <Container className="lab-page" as="main" maxW="5xl" paddingX="lg" paddingY="lg" lang={locale}>
      <Stack gap="lg">
        <Stack gap="xs">
          <Text textStyle="label/XS/medium" color="fg.accent-primary">
            {t("webview.labPage.eyebrow", "Extension Lab")}
          </Text>
          ...
          <Text textStyle="paragraph/M/regular" color="fg.muted" maxW="2xl">
            {t(
              "webview.labPage.description",
              "A self-contained React surface for trying out the extension API. The counter runs through extension commands and persists per project with ctx.storage.",
            )}
          </Text>
        </Stack>
