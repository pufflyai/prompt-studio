...
The final extension model separates **installed extension source** from **project extension enablement**:

```txt
Installed extension source
  Editable source under ~/.pstdio/extensions/<install-name>

Project extension instance
  A project-level enabled/disabled/configured reference to installed source
```

Extensions are source-installed so users and coding agents can inspect, modify, and extend them locally. Projects toggle installed extensions on or off. If two projects need different behavior, the user copies the extension source, modifies the copy, and enables the desired variant per project.
...
- `pstdio add <extension>` installs extension source into the Prompt Studio user root.
- Projects enable, disable, remove, and configure installed extensions.
...
- `pstdio add <source>` creates or updates an installed extension source.
...
- **Purpose:** project-level enablement, config, and diagnostics for an installed extension.

This is the per-project toggle/config layer.
...
- Removing a project extension instance should not delete installed source.
...
| -------------- | ------: | -------: | ------------------------------------------------------------------------- |
| `project_id`   |  `text` |       no | References `projects.id`                                                  |
| `extension_id` |  `text` |       no | Extension id, e.g. `pstdio.planner`                                       |
| `namespace`    |  `text` |       no | Cached namespace for diagnostics and inspection                           |
...
```txt
(project_id, extension_id, scope_type, scope_id, key)
```
...
(project_id, extension_id, scope_type, scope_id)
```
...
| -------------- | ------: | -------: | ------------------------------ |
| `project_id`   |  `text` |       no | References `projects.id`       |
| `extension_id` |  `text` |       no | Extension id                   |
| `namespace`    |  `text` |       no | Cached namespace               |
...
```txt
(project_id, extension_id, scope_type, scope_id, collection, item_id)
```

Recommended indexes:

```txt
(project_id, extension_id, collection)
(project_id, extension_id, scope_type, scope_id, collection)
```
...
| -------------- | --------: | -------: | ------------------------------- |
| `project_id`   |    `text` |       no | References `projects.id`        |
| `extension_id` |    `text` |       no | Provider extension              |
...
```txt
(project_id, extension_id, template_key)
```
...
```txt
(project_id, extension_id, skill_key)
```
...
```txt
(project_id, extension_id, schedule_key)
```
...
| Field                 |                               Type | Nullable | Notes                                           |
| --------------------- | ---------------------------------: | -------: | ----------------------------------------------- |
| `id`                  |                             `text` |       no | Template id                                     |
| `project_id`          |                             `text` |      yes | Null for global/default records where supported |
| `template_type`       |                             `text` |       no | Extension-owned or kernel-owned template type   |
| `name`                |                             `text` |       no | Existing display/key field                      |
...
`pstdio add <source>` installs extension source into the Prompt Studio user root.
...
```bash
pstdio add planner
pstdio add extension-lab
pstdio add github.com/prompt-studio/prompt-studio/extensions/planner
pstdio add ./my-extension
```

Recommended behavior:

1. Resolve the extension source from catalog, GitHub, local path, artifact, or manual source.
2. Copy source into `~/.pstdio/extensions/<install-name>`.
3. Load the extension manifest from source.
4. Create or update `installed_extension_sources`.
5. If running inside a project, create or enable `project_extension_instances`.
6. Run diagnostics.
7. Print available canonical commands.
...
Commands:
  pstdio planner tickets create
  pstdio planner tickets list
  pstdio planner tickets pull
  pstdio planner tickets push
...
| `remove`  | Removes the project extension instance; does not delete installed source             |
| `check`   | Runs extension loading and registry diagnostics                                      |
| `add`     | Installs source and usually enables it for the current project                       |
...
- `installed_extension_sources` -> `project_extension_instances`: one-to-many.
- `projects` -> `project_extension_instances`: one-to-many.
- `project_extension_instances` -> `extension_kv` / `extension_collection_items`: logical one-to-many through `(project_id, extension_id)`.
...
- Add project extension instance/config storage.
...
- Removing a project extension instance does not delete installed source.
- Purging extension data must be explicit.
