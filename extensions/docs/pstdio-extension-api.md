...
This is the reason `pstdio add` pulls source instead of only installing opaque packages: coding agents and users can inspect, edit, and extend extension source locally.

### `pstdio add`

`pstdio add` installs extension source into the Prompt Studio root and enables it for the current project when a project is resolved.

```bash
pstdio add planner
pstdio add workspace-shell
pstdio add github.com/prompt-studio/prompt-studio/extensions/planner
pstdio add ./local-extension
```

Short names resolve through an extension catalog. First-party extensions can be published from the Prompt Studio repository under:

```txt
prompt-studio/extensions/<extension-name>
```

The catalog may be backed by versioned artifacts rather than raw GitHub trees. The local result is still source code in `~/.pstdio/extensions/<extension-name>`.

Expected flow:

```txt
pstdio add planner
  -> resolve source from catalog or explicit path
  -> copy source to ~/.pstdio/extensions/planner
  -> load extension manifest
  -> register or update the local source record
  -> enable it for the current project if inside one
  -> run diagnostics
...
The project stores enablement/configuration for root-installed extensions. Source is not copied into the project by default.
...
The project tracks enabled extension instances:

```ts
type ProjectExtensionInstance = {
  projectId: string;
  extensionId: string;
  namespace: string;
  sourceName: string;
...
Project instance state is API-owned. The CLI and dashboard read/write it through the API.
...
```ts
import { defineExtension, packageAsset, params } from "@pstdio/sdk";
...
Extension storage is API-owned and namespaced by project and extension.
