...
type FileListTreeNodeState = {
  isBranch: boolean;
  expanded: boolean;
};

interface FileListTreeNodeProps {
  node: ChangedFileTreeNode;
  nodeState: FileListTreeNodeState;
  viewMode: ChangedFilesViewMode;
  selectedPath: string | null;
  onSelectPath: (path: string) => void;
  resolveFileIcon?: (path: string) => FileIconInfo;
...
const FileListFolderNode = (props: { node: ChangedFileTreeNode; expanded: boolean; depth: number }) => {
  const { node, expanded, depth } = props;
...
const FileListFileText = (props: { filePath: string; nodeName: string; viewMode: ChangedFilesViewMode }) => {
  const { filePath, nodeName, viewMode } = props;
  ...
  if (!useFlatDisplay) return nodeName;
...
const FileListFileNode = (props: {
  ...
  isSelected: boolean;
  onSelectPath: (path: string) => void;
  fileIcon: FileIconInfo;
}) => {
  const { node, filePath, depth, viewMode, isSelected, onSelectPath, fileIcon } = props;
  ...
  return (
    ...
    >
      ...
      <TreeView.ItemText textStyle="paragraph/XS/regular" truncate minW="0">
        <FileListFileText filePath={filePath} nodeName={node.name} viewMode={viewMode} />
      </TreeView.ItemText>
    </TreeView.Item>
  );
};

const FileListTreeNode = (props: FileListTreeNodeProps) => {
  const { node, nodeState, viewMode, selectedPath, onSelectPath, resolveFileIcon } = props;
  const depth = viewMode === "nested" ? getPathDepth(node.id) : 0;

  if (nodeState.isBranch) {
    return <FileListFolderNode node={node} expanded={nodeState.expanded} depth={depth} />;
  }
          ...
          >
            <TreeView.Tree w="full" minW="0">
              <TreeView.Node
                render={(renderProps) => (
                  <FileListTreeNode
                    node={renderProps.node}
                    nodeState={renderProps.nodeState}
                    viewMode={viewMode}
