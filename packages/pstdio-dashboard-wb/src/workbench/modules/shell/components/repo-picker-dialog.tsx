...
import { type DirectoryEntry, listDirectory } from "../data/file-system-api";
...
interface LoadFolderDirectoryInput {
  path?: string;
  t: (key: string) => string;
...
const loadFolderDirectory = async (input: LoadFolderDirectoryInput) => {
  const {
    path,
    t,
    setIsLoading,
...
export const RepoPickerDialog = (props: RepoPickerDialogProps) => {
  ...
  return (
    <RepoPickerDialogUI
      ...
      onSelectHomeDirectory={() => loadPath("~")}
      onSelectParentDirectory={() => {
        if (currentPath) loadPath(resolveParentPath(currentPath));
      }}
