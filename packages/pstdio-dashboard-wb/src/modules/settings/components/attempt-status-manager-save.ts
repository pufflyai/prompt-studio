...
interface SaveInput {
  ...
  createStatus: (input: { name: string; color: string; icon?: string | null }) => Promise<unknown>;
...
export const saveAttemptStatuses = async (input: SaveInput) => {
  ...
  for (const draft of drafts) {
    if (draft.isNew) {
      await input.createStatus({ name: draft.name, color: draft.color, icon: draft.icon });
      continue;
    }
