...
const isProjectSettingsPanel = (panel: DashboardExtensionSettingsPanelRecord) =>
...
export const buildDashboardSettingsTree = (input: BuildDashboardSettingsTreeInput) => {
  ...
  const projectExtensionSettingsPanels = extensionSettingsPanels.filter(isProjectSettingsPanel);

  if (projectExtensionSettingsPanels.length > 0) {
    sections.splice(1, 0, {
      id: "extension-settings",
      label: "Extensions",
      nodes: projectExtensionSettingsPanels.map(extensionSettingsPanelNode),
    });
  }
