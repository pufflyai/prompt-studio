...
describe("dashboard workbench extension contribution module", () => {
  test("refreshes cached extension metadata when installed extension sync rows change", async () => {
    const metadataResponses = [metadata, emptyMetadata];
    const loadMetadata = mock(async () => metadataResponses.shift() ?? emptyMetadata);
    const workbench = createWorkbenchCore();

    registerProjectMode(workbench);
    workbench.context.set(dashboardSelectedProjectIdContextKey, "project-1");
    const disposable = workbench.registerModule(createExtensionContributionsModule({ loadMetadata }));

    try {
      await flushMicrotasks();

      expect(getCachedDashboardExtensionMetadata("project-1")?.extensions).toHaveLength(1);

      getWriter("installed_extension_sources")?.upsert({
        id: "installed-source-1",
        install_name: "extension-lab",
      });
      await flushMicrotasks();

      expect(loadMetadata).toHaveBeenCalledTimes(2);
      expect(getCachedDashboardExtensionMetadata("project-1")?.extensions).toEqual([]);
    } finally {
      disposable.dispose();
