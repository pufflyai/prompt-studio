...
describe("dashboard project switching", () => {
  ...
  test("returns to project selection when the selected project is deleted", async () => {
    const workbench = await createWorkbench("project-1");
    const projectsWriter = getWriter("projects");

    if (!projectsWriter) throw new Error("Missing projects writer");

    await workbench.resources.openResource(dashboardSettingsResources.dangerZone, { replaceActive: true });
    markInitialCollectionsSyncComplete();
    projectsWriter.truncateAndWrite([
      {
        id: "project-1",
        name: "Prompt Studio",
        created_at: "2026-05-20T08:00:00Z",
        updated_at: "2026-05-20T09:00:00Z",
        deleted_at: "2026-05-30T10:00:00Z",
      },
    ]);

    expect(workbench.context.get(dashboardSelectedProjectIdContextKey)).toBeUndefined();
    expect(workbench.modes.getActiveModeId()).toBe("project-selection");
    expect(getAreaContributionIds(workbench, "overlay")).toEqual([dashboardWidgetIds.projectPicker]);
    expect(getAreaContributionIds(workbench, "main")).toEqual([]);
  });
});
