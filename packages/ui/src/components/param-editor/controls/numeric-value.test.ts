...
import { clampToRange, formatUnitValue, parseUnitValue, resolveNumericValue, snapToStep } from "./numeric-value";
...
describe("resolveNumericValue", () => {
  it("clamps continuous values without snapping", () => {
    expect(resolveNumericValue({ value: 11, min: 0, max: 10, step: 5 })).toBe(10);
  });

  it("snaps discrete values to the step grid and clamps", () => {
    expect(resolveNumericValue({ value: 8, min: 0, max: 100, step: 5, discrete: true })).toBe(10);
    expect(resolveNumericValue({ value: 999, min: 0, max: 100, step: 5, discrete: true })).toBe(100);
  });
});
...
describe("parseUnitValue", () => {
  it("parses values with and without the unit suffix", () => {
    expect(parseUnitValue("72%", "%")).toBe(72);
    expect(parseUnitValue("  42 ")).toBe(42);
  });

  it("returns null for non-numeric input", () => {
    expect(parseUnitValue("abc")).toBeNull();
  });
});
