...
export const resolveChatInputKeyboardAction = (state: ChatInputActionState): ChatInputAction => {
  const { canSubmit, hasQuestionPrompt, isDisabled, streaming, text } = state;
  if (isDisabled || !canSubmit || (streaming && !hasQuestionPrompt) || !hasMessageText(text)) return "none";
  return "submit";
};
...
export const resolveChatInputButtonAction = (state: ChatInputActionState): ChatInputAction => {
  ...
  if (!canSubmit || !hasMessageText(text)) return "none";
  return "submit";
};
