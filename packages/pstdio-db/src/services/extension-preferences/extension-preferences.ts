...
import { extension_skill_preferences, extension_template_preferences } from "../../db/schemas.pg";

const nowTimestamp = () => new Date().toISOString();

type TemplatePrefRow = typeof extension_template_preferences.$inferSelect;
type SkillPrefRow = typeof extension_skill_preferences.$inferSelect;

export type SetTemplatePreferenceInput = {
  project_id: string;
  extension_instance_id: string;
  template_key: string;
  enabled?: boolean;
  display_name_override?: string | null;
  metadata_json?: TemplatePrefRow["metadata_json"];
};
...
export const createExtensionTemplatePreferencesDBService = (db: DbClient) => {
  const list = async (projectId: string, extensionInstanceId?: string) => {
    const conditions = [eq(extension_template_preferences.project_id, projectId)];
    if (extensionInstanceId) {
      conditions.push(eq(extension_template_preferences.extension_instance_id, extensionInstanceId));
    }
    return db
      .select()
  ...
  const get = async (projectId: string, extensionInstanceId: string, templateKey: string) => {
    const [row] = await db
      .select()
      .from(extension_template_preferences)
      .where(
        and(
          ...
          eq(extension_template_preferences.extension_instance_id, extensionInstanceId),
  ...
  const set = async (input: SetTemplatePreferenceInput) => {
    const timestamp = nowTimestamp();
    const existing = await get(input.project_id, input.extension_instance_id, input.template_key);
    ...
    if (existing) {
      await db
        ...
        .where(
          and(
            eq(extension_template_preferences.project_id, input.project_id),
            eq(extension_template_preferences.extension_instance_id, input.extension_instance_id),
            eq(extension_template_preferences.template_key, input.template_key),
          ),
        );
    ...
    await db.insert(extension_template_preferences).values(row);
    return row;
  };

  const remove = async (projectId: string, extensionInstanceId: string, templateKey: string) => {
    const deleted = await db
      .delete(extension_template_preferences)
      .where(
        and(
          eq(extension_template_preferences.project_id, projectId),
          eq(extension_template_preferences.extension_instance_id, extensionInstanceId),
      ...
      .returning({ template_key: extension_template_preferences.template_key });
