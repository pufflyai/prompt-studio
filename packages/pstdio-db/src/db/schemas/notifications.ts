...
import type { JsonObject, ResourceRef } from "./types";
...
export const notifications = pgTable(
  "notifications",
  {
    ...
    target_json: jsonb("target_json").$type<ResourceRef | null>(),
    related_json: jsonb("related_json").$type<ResourceRef[]>().notNull().default([]),
    actions_json: jsonb("actions_json").$type<unknown[]>().notNull().default([]),
    metadata_json: jsonb("metadata_json").$type<JsonObject>(),
