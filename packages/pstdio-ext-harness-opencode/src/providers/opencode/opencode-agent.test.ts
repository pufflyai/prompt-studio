...
import { createOpencodeHarnessProvider } from "./opencode-agent";
...
describe("createOpencodeHarnessProvider", () => {
  test("returns a harness provider with id and label", () => {
    const a = agent();

    expect(a.id).toBe("pstdio.harness.opencode");
    expect(a.label).toBe("OpenCode");
  });

  test("supports session reattach", () => {
    const a = agent();

    expect(typeof a.reattachSession).toBe("function");
  });
});
...
describe("detect", () => {
  test("returns available when command is available", async () => {
    const a = agent({ available: true });

    expect(await a.detect?.({} as never)).toEqual({ available: true });
  });

  test("returns unavailable when command is unavailable", async () => {
    const a = agent({ available: false });

    expect(await a.detect?.({} as never)).toEqual({
      available: false,
      reason: "OpenCode executable was not found.",
    });
  });
});
...
describe("getMessages", () => {
  test("normalizes messages from service", async () => {
    ...
    const a = createOpencodeHarnessProvider(
...
describe("timeoutStrategy", () => {
  test("startSession returns process with provider timeout strategy", async () => {
    ...
    const a = createOpencodeHarnessProvider(agentDefaults(), { ...serviceOverrides(), fetcher });
...
describe("reattachSession", () => {
  ...
  test("exposes the reattach session handler", () => {
    const a = agent();
    expect(a.reattachSession).toBeDefined();
  });
});
