...
describe("known-agents", () => {
  ...
  test("harnessLocalId strips the extension namespace", () => {
    expect(harnessLocalId("pstdio.pstdio-claude-code.claude-code")).toBe("claude-code");
    expect(harnessLocalId("claude-code")).toBe("claude-code");
  });

  test("findAgent reconciles namespaced harness ids via their local id", () => {
    expect(findAgent("pstdio.pstdio-claude-code.claude-code")?.binary).toBe("claude");
    expect(findAgent("pstdio.pstdio-opencode.opencode")?.binary).toBe("opencode");
