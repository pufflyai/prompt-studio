...
describe("workspacesForTicket", () => {
  it("lists workspaces for a ticket by ticket shorthand", async () => {
    const ctx = {
      projectId: "proj-1",
      client: {
        tickets: {
    ...
    const workspaces = await workspacesForTicket(ctx, { ticketId: "PS-1" });

    expect(workspaces.map((workspace) => workspace.id)).toEqual(["ws-1", "ws-3"]);
  });
  ...
  it("returns an empty list when the ticket cannot be resolved", async () => {
    ...
    const workspaces = await workspacesForTicket(ctx, { ticketId: "PS-99" });
