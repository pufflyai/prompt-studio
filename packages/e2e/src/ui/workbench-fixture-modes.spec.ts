...
const createProject = async (request: import("@playwright/test").APIRequestContext, folderPath?: string) => {
  const response = await request.post(`${apiBase}/v1/projects`, {
    data: folderProjectInput({ name: "Extension Lab Modes" }, folderPath),
  });
  ...
  const selectResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith(
        "/extensions/commands/pstdio.workbench-fixture.command.cams.select/execute",
...
test("artifacts are created from the panel menu and inspected in the Side Panel", async ({ page, request }) => {
  ...
  const createResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith(
        "/extensions/commands/pstdio.workbench-fixture.view.artifact-create.controls.onValueChange/execute",
      ) && response.request().method() === "POST",
  );
  ...
  const deleteResponse = page.waitForResponse(
    (response) =>
      new URL(response.url()).pathname.endsWith(
        "/extensions/commands/pstdio.workbench-fixture.command.glass-lab-artifacts.delete/execute",
      ) && response.request().method() === "POST",
  );
