...
describe("pstdio projects startup-script", () => {
  test("set stores script from file", () => {
  ...
  test("get shows message when no script configured", () => {
  ...
  test("clear removes the startup script", () => {
  ...
  test("full lifecycle: set → get → clear → get", () => {
  ...
  test("set fails outside a pstdio project", () => {
    const repo = createGitRepo();
    dirs.push(repo);
    const scriptPath = join(repo, "setup.sh");
