...
export const toCreateFields = (record: WorkbenchExtensionKanbanRendererRecord) =>
  Object.entries(record.createRow?.params ?? {}).flatMap(([id, descriptor]) => {
    const field = toCreateField(id, descriptor);
    return field ? [field] : [];
  });

export const editableAttributeValues = (submission: KanbanRendererCreateSubmission) =>
