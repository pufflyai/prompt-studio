...
import type { EnumOptions, KanbanRendererCreateField, KanbanRendererCreateFieldType, ResourceRef } from "../../core";
import type {
...
const isExtensionResource = (resource: unknown): resource is ResourceRef =>
  Boolean(
    resource &&
...
export const defaultResolveRowResource = (_: WorkbenchExtensionKanbanRendererRecord, row: KanbanRendererRow) => {
  const { resource } = row;
  if (isExtensionResource(resource)) return resource;
  return undefined;
};
...
const toCreateField = (
  ...
  const options =
    "options" in descriptor && Array.isArray(descriptor.options)
      ? (descriptor.options as Array<{
          value: string;
          label: string;
          icon?: string;
        }>)
      : [];
  const optional = descriptor as {
    placeholder?: unknown;
    multiple?: unknown;
    accept?: unknown;
  };
  ...
  return executeWorkbenchExtensionCommand(context, commandId, {
    params: {
      renderer: {
        ...
        projectId: context.projectId,
        ...(commandResource ? { resource: commandResource } : {}),
