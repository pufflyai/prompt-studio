...
const workspaceA: ResourceRef = {
  type: "workspace",
  id: "pstdio://workspace/a",
};
const workspaceB: ResourceRef = {
  type: "workspace",
  id: "pstdio://workspace/b",
};
const terminal: ResourceRef = {
  type: "terminal",
  id: "pstdio://terminal/a",
};
...
describe("createPrimaryCoordinator", () => {
  ...
  test("never clears the primary anchor itself", () => {
    ...
    expect(layout.getLayout().regions.main.widgets[0].resourceKey).toBe(resourceKey(workspaceB));
  });
  ...
  test("keeps a derived anchor populated for the new primary during the primary change", () => {
    ...
    const populateForNewPrimary = layout.store.subscribe((state, previous) => {
      const primary = resourceKey(getAnchorResource(state.layout, "primary"));
      if (primary === resourceKey(getAnchorResource(previous.layout, "primary")) || primary !== resourceKey(workspaceB))
        return;
      layout.openWidget("terminals", { resource: terminal });
    });
    createPrimaryCoordinator({ layout, isInScope: () => true });
