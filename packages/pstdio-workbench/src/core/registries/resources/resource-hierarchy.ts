...
export const walkResourceHierarchy = (
  ...
  const visitedKeys = new Set([`resource:${resourceKey(resource)}`]);
  let current = resource;
  while (true) {
    const provider = sorted.find((candidate) => candidate.canResolve(current));
    const parent = provider?.getParent(current);
    if (!parent) break;
    const parentKey = isWorkbenchViewHierarchyNode(parent)
      ? `view:${parent.viewId}`
      : `resource:${resourceKey(parent)}`;
    if (visitedKeys.has(parentKey)) {
      ...
      notifyCycle({
        ...
        path: [...path],
        repeatedUri: isWorkbenchViewHierarchyNode(parent) ? parentKey : resourceKey(parent),
      });
      break;
    }
    path.unshift(parent);
