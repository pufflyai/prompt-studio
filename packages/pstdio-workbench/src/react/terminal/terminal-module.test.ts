...
describe("restored terminal selection", () => {
  ...
  test("restores a real terminal after a persisted layout scope is selected", async () => {
    const savedLayouts = new Map<string | undefined, WorkbenchLayout>();
    const persistence = {
      getLayout: (scope?: string) => savedLayouts.get(scope),
      setLayout: (layout: WorkbenchLayout, scope?: string) => savedLayouts.set(scope, structuredClone(layout)),
    };
    const firstWorkbench = createWorkbenchCore({ layoutPersistence: persistence });
    firstWorkbench.registerModule(createWorkbenchTerminalModule());
    firstWorkbench.layout.setPersistenceScope("project/project-1");
    await firstWorkbench.commands.executeCommand(WORKBENCH_TERMINAL_OPEN_COMMAND_ID);
    firstWorkbench.layout.setRegionActiveWidget("secondary", WORKBENCH_TERMINAL_LAUNCHER_WIDGET_ID);

    const restoredWorkbench = createWorkbenchCore({ layoutPersistence: persistence });
    restoredWorkbench.registerModule(createWorkbenchTerminalModule());
    restoredWorkbench.layout.setPersistenceScope("project/project-1");
    const secondary = restoredWorkbench.layout.getLayout().regions.secondary;
    const restoredTerminal = secondary.widgets.find(
      (placement) => placement.contributionId === WORKBENCH_TERMINAL_WIDGET_ID,
    );
  ...
  test("restores another secondary panel when a scoped layout persisted the hidden launcher active", () => {
    const savedLayouts = new Map<string | undefined, WorkbenchLayout>();
    const persistence = {
      getLayout: (scope?: string) => savedLayouts.get(scope),
      setLayout: (layout: WorkbenchLayout, scope?: string) => savedLayouts.set(scope, structuredClone(layout)),
    };
    const registerReviewPanel = (workbench: ReturnType<typeof createWorkbenchCore>) => {
      workbench.layout.registerPanel({
        id: "test.review-checks",
        title: "Review checks",
        region: "secondary",
        rendererId: "test.review-checks",
        closable: true,
      });
    };
    const firstWorkbench = createWorkbenchCore({ layoutPersistence: persistence });
    firstWorkbench.registerModule(createWorkbenchTerminalModule());
    registerReviewPanel(firstWorkbench);
    firstWorkbench.layout.setPersistenceScope("project/project-1/mode/review");
    const reviewPanel = firstWorkbench.layout.openPanel("test.review-checks", { strategy: { kind: "persistent" } });
    firstWorkbench.layout.setRegionActiveWidget("secondary", WORKBENCH_TERMINAL_LAUNCHER_WIDGET_ID);

    const restoredWorkbench = createWorkbenchCore({ layoutPersistence: persistence });
    restoredWorkbench.registerModule(createWorkbenchTerminalModule());
    registerReviewPanel(restoredWorkbench);
    restoredWorkbench.layout.setPersistenceScope("project/project-1/mode/review");

    expect(restoredWorkbench.layout.getLayout().regions.secondary.activeWidgetId).toBe(reviewPanel.instanceId);
  });
});
