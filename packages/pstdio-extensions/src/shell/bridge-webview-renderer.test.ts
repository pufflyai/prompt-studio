...
const setupWebviewWidget = () => {
  ...
  const widget = shell.layout.getWidget("lab.page")!;
  const placement: ShellWidgetPlacement = { widgetId: "lab.page", contributionId: "lab.page" };

  return { shell, widget, placement };
};
...
describe("createBridgeWebviewRenderer", () => {
  test("uses the injected createHostCapabilities factory", () => {
    ...
    const calls: { shell: unknown; webviewId: string }[] = [];
