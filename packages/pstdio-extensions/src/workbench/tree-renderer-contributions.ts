...
const treeTargetAreas = {
  "workbench.main": "main",
  "workbench.main.left": "main-left",
  "workbench.main.right": "main-right",
  "workbench.secondary": "secondary",
} satisfies Partial<Record<NonNullable<ExtensionTreeViewRecord["target"]>, WorkbenchArea>>;

const text = (value: Localizable<string> | undefined, fallback = "") => {
  if (value === undefined) return fallback;
  if (!isLocalizedString(value)) return value;
  return value.default ?? value.$l10n;
};

const unwrapCommandValue = (result: unknown) => {
...
const registerTreeViewWidget = (input: RegisterWorkbenchExtensionTreeRenderersInput, view: ExtensionTreeViewRecord) => {
  ...
  const area = treeTargetAreas[view.target ?? "workbench.main"] ?? "main";
