...
const preservedRuntimeDirs: string[] = [];
let previousPackagePath: string | undefined;
let previousPreserveRuntime: string | undefined;
...
afterEach(() => {
  ...
  if (previousPreserveRuntime === undefined) delete process.env.PSTDIO_EXTENSION_RUNTIME_PRESERVE_TEMP;
  else process.env.PSTDIO_EXTENSION_RUNTIME_PRESERVE_TEMP = previousPreserveRuntime;

  previousPackagePath = undefined;
  previousPreserveRuntime = undefined;
  for (const dir of tempDirs) rmSync(dir, { recursive: true, force: true });
  for (const dir of preservedRuntimeDirs) rmSync(dir, { recursive: true, force: true });
  tempDirs.length = 0;
  preservedRuntimeDirs.length = 0;
});
...
test("preserves runtime package files for Bun watch mode", async () => {
  const dir = createTempDir();
  writePackage(dir);
  writeFileSync(
    join(dir, "extension.ts"),
    `
      ...
      writeFileSync(join(packagePath, "runtime-root.txt"), fileURLToPath(new URL("..", import.meta.url)));
  ...
  previousPackagePath = process.env.PSTDIO_LOADER_TEST_PACKAGE_PATH;
  previousPreserveRuntime = process.env.PSTDIO_EXTENSION_RUNTIME_PRESERVE_TEMP;
  process.env.PSTDIO_LOADER_TEST_PACKAGE_PATH = dir;
  process.env.PSTDIO_EXTENSION_RUNTIME_PRESERVE_TEMP = "1";

  const diagnostics: ExtensionDiagnostic[] = [];
  await loadExtensionPackage({ path: dir }, diagnostics);

  const runtimeRoot = readFileSync(join(dir, "runtime-root.txt"), "utf8").trim();
  preservedRuntimeDirs.push(runtimeRoot);
