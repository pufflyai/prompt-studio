...
describe("createCommandRunner: lifecycle", () => {
  test("runs a command and emits requested/started/completed lifecycle events", async () => {
    ...
    const runner = makeRunner({
      ...
      hooks: {
        observeAll: {
          event: "command.completed:lab.counter.bump",
          handler: async () => {
            events.push("completed");
          },
        },
        observeStarted: {
          event: "command.started:lab.counter.bump",
          handler: async () => {
            events.push("started");
          },
        },
        observeRequested: {
          event: "command.requested:lab.counter.bump",
          handler: async () => {
  ...
  test("returns command_not_found when the id is unknown", async () => {
    const runner = makeRunner({ id: "pstdio.lab", namespace: "lab", name: "Lab" });
...
describe("createCommandRunner: hooks and nesting", () => {
  test("hook errors are isolated and don't fail the command", async () => {
    const runner = makeRunner({
      ...
      hooks: {
        crashy: {
          event: "command.completed:lab.ping",
          handler: async () => {
