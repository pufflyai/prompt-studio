...
import { createClaudeCodeHarnessProvider } from "./claude-code";

const buildTranscript = (entries: unknown[]) => entries.map((entry) => JSON.stringify(entry)).join("\n");

describe("createClaudeCodeHarnessProvider getMessages", () => {
  test("keeps the latest entry for duplicate UUIDs", async () => {
    ...
    const agent = createClaudeCodeHarnessProvider({
      isCommandAvailable: () => true,
      readTranscript: async () => transcript,
    });

    const messages = await agent.getMessages("session-1");
  ...
  test("normalizes transcript tool_result entries without creating user tool messages", async () => {
    ...
    const toolPart = toolMessage?.parts[0] as {
      type: string;
      status?: string;
      callId?: string;
    };
