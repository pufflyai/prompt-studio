import { CloseButton, Dialog, Flex, HStack, Icon, Input, InputGroup, Menu, Stack, Text } from "@chakra-ui/react";
import { ScrollArea, type ThemePreference, themePreferences, useThemePreference } from "@pstdio/ui";
...
const getThemeEntryIndex = (preference: ThemePreference) => {
  return Math.max(themePreferences.indexOf(preference), 0);
};
...
export const CommandPalette = (props: CommandPaletteProps) => {
  ...
  const beginThemePreview = () => {
    ...
    setView("theme");
    setQuery("");
    setActiveIndex(getThemeEntryIndex(themePreference));
  };
  ...
  const entries = buildCommandPaletteEntries({
    ...
    labels: {
      ...
      themeLabel: (preference) => t(`common:themes.${preference}`),
    },
    run: handleRun,
  });
  ...
  }, [initialView, open, themePreference]);
                  ...
                  {filteredEntries.length > 0 ? (
                    filteredEntries.map((entry, index) => (
                      <Menu.Item
                        key={entry.id}
                        value={entry.id}
                        bg={index === activeIndex ? "bg.menu-item.hover" : "bg.menu-item.default"}
                      ...
                      >
                        <Flex justifyContent="space-between" alignItems="center" gap="xs" flex="1" minW="0">
                          <HStack gap="2" minW="0">
                            <Icon as={entry.icon} boxSize="14px" color="fg.muted" />
                            <Stack gap="0" minW="0">
