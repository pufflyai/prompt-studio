...
const moveWorkspaceEntry = async (
  ...
  const treeState = ctx.treeViews.getTreeState(dashboardWidgetIds.workspaceFileTree);
  ...
  for (const path of expandedSourcePaths) {
    ctx.treeViews.setNodeExpanded(dashboardWidgetIds.workspaceFileTree, path, false);
  }
  for (const path of new Set(movedExpandedPaths)) {
    ctx.treeViews.setNodeExpanded(dashboardWidgetIds.workspaceFileTree, path, true);
  }
  if (selectedPath !== treeState.selectedNodeId) {
    ctx.treeViews.setSelectedNode(dashboardWidgetIds.workspaceFileTree, selectedPath);
  }
...
export const registerWorkspaceFileContributions = (ctx: WorkbenchModuleContext) => {
  ...
  const treeActions: WorkspaceFileTreeActions = {
    beginCreate: (resource, parentPath, type) => {
      ...
      if (parentPath) ctx.treeViews.setNodeExpanded(dashboardWidgetIds.workspaceFileTree, parentPath, true);
      ctx.views.refreshView(dashboardWidgetIds.workspaceFileTree);
    },
    cancelCreate: () => {
