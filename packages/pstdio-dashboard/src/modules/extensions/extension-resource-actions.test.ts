...
test("preserves resource ownership when resolving row actions", () => {
  const resource = {
    type: "ticket",
    id: "ticket-1",
    extensionId: "pstdio.pstdio-planner",
    projectId,
    metadata: { documentId: "notes" },
  };
  expect(toDashboardExtensionResource(resource, projectId)).toMatchObject(resource);
});
