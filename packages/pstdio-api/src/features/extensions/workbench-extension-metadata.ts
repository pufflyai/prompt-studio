...
const toDocumentEditorRecord = (
  editor: ExtensionRuntime["documentEditors"][number],
): WorkbenchExtensionDocumentEditorRecord | null => {
  const readCommandId = refIdOf(editor.contribution.readCommand);
  if (!readCommandId) return null;

  return {
    id: editor.id,
    extensionId: editor.extensionId,
    title: editor.contribution.title,
    resourceKind: editor.contribution.resourceKind,
    readCommandId,
    updateCommandId: refIdOf(editor.contribution.updateCommand),
    layout: editor.contribution.layout as WorkbenchExtensionDocumentEditorRecord["layout"],
  };
};
...
export const buildWorkbenchExtensionMetadata = (
  ...
  return {
    ...
    documentEditors: compact(runtime.documentEditors.map(toDocumentEditorRecord)),
