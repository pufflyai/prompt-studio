    ...
    const runtime = normalizeExtensionSources([
      {
        ...
        manifest: {
          ...
          description: "Built-in sessions workbench mode.",
          version: "1.0.0",
          publisher: "pstdio",
          main: "./extension.ts",
          enginesPstdio: "^1.0.0",
...
describe("buildWorkbenchExtensionMetadata document editors", () => {
  test("includes workbench document editor contributions", () => {
    const runtime = normalizeExtensionSources([
      {
        sourcePath: "/extension/extension.ts",
        sourceKind: "local_path",
        ...
        definition: {
          documentEditors: {
            ticket: {
              title: "Ticket",
              resourceKind: "ticket",
              readCommand: "planner.ticketDocument.read",
              updateCommand: "planner.ticketDocument.update",
              layout: {
                autoSave: true,
    ...
    expect(metadata.documentEditors).toEqual([
      {
        id: "planner.ticket",
        extensionId: "pstdio.planner",
        title: "Ticket",
        resourceKind: "ticket",
        readCommandId: "planner.ticketDocument.read",
        updateCommandId: "planner.ticketDocument.update",
        layout: {
          autoSave: true,
          header: { visible: false },
          properties: { target: "workbench.main.left" },
