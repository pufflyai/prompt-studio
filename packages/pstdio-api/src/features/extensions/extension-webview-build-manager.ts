...
const defaultRunCommand: BuildCommandRunner = (file, args, options) =>
  new Promise((resolveResult) => {
    const child = nodeSpawn(file, args, { cwd: options.cwd, env: options.env, stdio: ["ignore", "pipe", "pipe"] });
    const stdout: Buffer[] = [];
    const stderr: Buffer[] = [];
...
export const createExtensionWebviewBuildManager = (
  ...
  const buildOnce = async (row: InstalledSourceWithManifest, webviewId: string, shellPath: string, distDir: string) => {
    const command = managedCommand(buildArgs(shellPath, distDir, false));
    const result = await runCommand(command.file, command.args, { cwd: row.source_path, env: command.env });
    if (result.exitCode === 0) {
      reportSuccess(row.install_name, webviewId);
      return true;
    }

    const details = result.stderr.trim() || result.stdout.trim();
    reportFailure(row.install_name, webviewId, buildFailure(row.install_name, webviewId, details));
    return false;
  };
  ...
  const startWatch = (
    ...
    const command = managedCommand(buildArgs(shellPath, distDir, true));
    const child = spawn(command.file, command.args, { cwd: row.source_path, env: command.env, stdio: "ignore" });
