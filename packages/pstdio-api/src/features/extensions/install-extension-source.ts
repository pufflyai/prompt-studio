...
import { linkUsableNodeModules, linkWorkspaceNodeModules } from "./install-extension-source-node-modules";
...
export const installExtensionSource = async (input: InstallExtensionSourceInput) => {
  ...
  try {
    ...
    if (resolvedSource.kind === "local") {
      if (input.skipInstall) linkUsableNodeModules(resolvedSource.path, installPath);
      else linkWorkspaceNodeModules(resolvedSource.path, installPath);
    }
