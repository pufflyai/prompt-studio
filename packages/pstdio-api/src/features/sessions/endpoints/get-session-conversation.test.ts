...
describe("GET /v1/sessions/:id/conversation", () => {
  test("returns the full conversation payload", async () => {
    ...
    const projectRes = await app.request("/v1/projects", {
      ...
      body: JSON.stringify(folderProjectInput({ name: "Conversation Download Project" })),
    });
  ...
  test("fetches messages from agent for completed sessions", async () => {
    ...
    const projectRes = await app.request("/v1/projects", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(folderProjectInput({ name: "Agent Messages Project" })),
    });
    const project = (await projectRes.json()) as { id: string };
  ...
  test("appends the persisted queued start prompt for queued sessions", async () => {
    const projectRes = await app.request("/v1/projects", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(folderProjectInput({ name: "Queued Conversation Project" })),
    });
    expect(projectRes.status).toBe(201);
    const project = (await projectRes.json()) as { id: string };
  ...
  test("preserves conversation history before the persisted queued follow-up prompt", async () => {
    const projectRes = await app.request("/v1/projects", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(folderProjectInput({ name: "Queued Follow-up Conversation Project" })),
    });
    expect(projectRes.status).toBe(201);
    const project = (await projectRes.json()) as { id: string };
  ...
  test("falls back to stored messages when a queued attachment is missing", async () => {
    const projectRes = await app.request("/v1/projects", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(folderProjectInput({ name: "Missing Attachment Project" })),
    });
    expect(projectRes.status).toBe(201);
    const project = (await projectRes.json()) as { id: string };
