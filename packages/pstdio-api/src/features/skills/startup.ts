...
type Deps = Pick<
  RouteDeps,
  "projectsService" | "reposService" | "skillsDbService" | "agentConfigsService" | "filesService"
>;
...
const installMissingSkillsForProject = async (deps: Deps, projectId: string, agents: { agent_id: string }[]) => {
  const [repos, skills] = await Promise.all([
    ...
    deps.skillsDbService.list(projectId),
  ]);
...
export const ensureSkillsInstalled = async (deps: Deps) => {
  const [projects, agents] = await Promise.all([deps.projectsService.list(), deps.agentConfigsService.list()]);
