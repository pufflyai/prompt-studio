...
import { ShellSessionAttachedPanel } from "../session-panel/session-panel";
...
export const ShellFloatingSessionPortal = (props: ShellFloatingSessionPortalProps) => {
  const { shell, hasFloatingPanel, activeSessionSlot, sessionHost } = props;

  if (!hasFloatingPanel || !activeSessionSlot || !sessionHost) return null;

  return createPortal(<ShellArea shell={shell} area="floating" title="Session" showHeader={false} />, sessionHost);
};
...
export const ShellFloatingSessionHeader = (props: ShellFloatingSessionHeaderProps) => {
  const { shell, hasFloatingHeader } = props;

  if (!hasFloatingHeader) return null;

  return (
    <Box alignItems="center" display="flex" flex="1" h="full" minW="0" overflow="hidden" w="full">
      <ShellArea shell={shell} area="floating-header" title="Floating header" showHeader={false} />
    </Box>
  );
};
