...
const MainHeaderBar = (props: MainHeaderBarProps) => {
  ...
  return (
    ...
    >
      ...
      <ShellAreaTabs shell={shell} area="main" />
...
const MainBottomSection = (props: MainBottomSectionProps) => {
  const { shell, hasMainBottomHeader, hasMainBottomContentTabs, bottomResize } = props;
      ...
      <Box as="section" minH="0" minW="0" overflow="hidden" display="flex" flexDirection="column">
        {hasMainBottomHeader || hasMainBottomContentTabs ? (
          <Header
            ...
            overflowY="hidden"
          >
            <ShellAreaTabs shell={shell} area="main-bottom" />
            {hasMainBottomHeader ? (
              <Box flex="1" h="full" minW="0" overflow="hidden">
                <ShellArea shell={shell} area="main-bottom-header" title="Main bottom header" showHeader={false} />
              </Box>
            ) : null}
          </Header>
        ) : null}
        <Box flex="1" minH="0" minW="0" overflow="hidden">
          <ShellArea shell={shell} area="main-bottom" title="Main bottom" />
        </Box>
      </Box>
    </>
  );
...
export const ShellWorkbenchBody = (props: ShellWorkbenchBodyProps) => {
  ...
  const layoutAreas = useShellStore(shell.layout.store, (state) => state.layout.areas);
  ...
  const mainArea = <ShellArea shell={shell} area="main" title="Main" showHeader={false} />;
  const mainAreaWithRightPanel = hasMainRight ? (
    <ResizableSplitLayout
      minH="0"
      minW="0"
      resizableSide="right"
      resizablePanel={<ShellRightSidePanel shell={shell} hasHeader={hasMainRightHeader} />}
