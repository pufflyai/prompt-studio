...
export interface ShellOverlayWidgetConfig {
...
const resolveOverlayConfig = (config: unknown): ShellOverlayWidgetConfig =>
  config && typeof config === "object" ? (config as ShellOverlayWidgetConfig) : {};

export const resolveOverlayDialogConfig = (placement: Pick<ShellWidgetPlacement, "closable">, config: unknown) => {
...
const resolveActivePlacement = (widgets: ShellWidgetPlacement[], activeWidgetId?: string) =>
  widgets.find((entry) => entry.widgetId === activeWidgetId) ?? widgets[0];

const ShellOverlayFallback = (props: { label: string }) => (
  <Center h="full" w="full" color="fg.muted" p="md" aria-label={props.label}>
    <ShellIcon name="circle-alert" size={20} />
  </Center>
);

interface ShellOverlayLayerProps {
  shell: ShellCore;
}

export const ShellOverlayLayer = (props: ShellOverlayLayerProps) => {
  const { shell } = props;
  const overlayArea = useShellStore(shell.layout.store, (state) => state.layout.areas.overlay);
  const renderers = useShellStore(shell.renderers.store, (state) => state.renderers);
  ...
  const body =
    widget && renderer ? (
      (renderer.render({ shell, widget, placement, refresh: () => undefined }) as ReactNode)
    ) : (
      <ShellOverlayFallback
        label={
  ...
  const handleOpenChange = (details: { open: boolean }) => {
    if (!details.open && canCloseOverlay) shell.layout.closeWidget(placement.widgetId);
  };
