...
export const dashboardFooterTreeViewId = "dashboard-shell.footer";
export const dashboardSettingsNavigationTreeViewId = "dashboard-shell.settings.navigation";

export const dashboardWidgetIds = {
  tickets: "dashboard-shell.tickets",
  extensionRoute: "dashboard-shell.extension-route",
  settings: "dashboard-shell.settings",
  status: "dashboard-shell.status",
  session: "dashboard-shell.session",
} as const;

const createResource = (kind: string, id: string, label: string, icon: string) =>
  ({
    kind,
    uri: `dashboard-shell://${kind}/${id}`,
    id,
    label,
    icon,
