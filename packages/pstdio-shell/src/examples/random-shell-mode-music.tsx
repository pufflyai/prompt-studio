...
const findActiveItem = (input: ShellWidgetRenderInput) => {
...
const playItem = (input: ShellWidgetRenderInput, item: ShellModeItem) => {
  input.shell.layout.openWidget(musicWidgetIds.player, {
...
const MusicTopBar = (props: { input: ShellWidgetRenderInput }) => {
...
const MusicPlayer = (props: { input: ShellWidgetRenderInput }) => {
  const item = findActiveItem(props.input);
  return (
    <Flex h="full" minH="0" align="center" justify="center" p="lg">
      <Stack gap="md" align="center" maxW="32rem">
        <Center
          aspectRatio={1}
          w="100%"
          maxW="20rem"
        ...
        {item.sections.map((section) => (
          <Stack key={section.heading} gap="2xs" align="center" maxW="24rem">
...
const MusicQueue = (props: { input: ShellWidgetRenderInput }) => {
  const { input } = props;
          ...
          <Stack gap="2xs">
            {musicMode.related.map((entry) => (
              <HStack key={entry.label} gap="xs">
                <ShellIcon name={entry.icon} size={12} color="fg.muted" />
...
const MusicControls = () => (
  <HStack h="full" px="md" gap="md" justify="center" bg="bg.subtle">
    <IconButton aria-label="Previous" variant="ghost" size="sm">
      <ShellIcon name="SkipBack" size={18} />
    </IconButton>
    ...
    <IconButton aria-label="Next" variant="ghost" size="sm">
      <ShellIcon name="SkipForward" size={18} />
    </IconButton>
    ...
    <HStack gap="xs">
      <Button size="xs" variant="ghost">
        <ShellIcon name="ListMusic" size={14} />
        Queue
      </Button>
      <Button size="xs" variant="ghost">
        <ShellIcon name="Heart" size={14} />
        Like
      </Button>
    </HStack>
  </HStack>
...
const MusicStatus = () => (
  <HStack h="full" px="sm" gap="md">
    <ShellIcon name={musicMode.statusIcon} size={12} color="fg.muted" />
...
const musicWidgets: WidgetSetup[] = [
  {
...
export const setupMusicMode = (ctx: ShellModeActivationContext): Disposable[] => {
