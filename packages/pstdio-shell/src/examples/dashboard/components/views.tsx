  ...
  return (
    ...
    >
      ...
      <Flex flex="1" minH="0" align="center" justify="center" direction="column" gap="2xs" p="md">
        <ShellIcon name={icon} size={24} color="fg.muted" />
          ...
          <Menu.Content minW="240px" bg="bg">
            <Menu.Item value="projects" asChild>
              <ListRow
                ...
                id="projects"
                label="Projects"
                icon={<ShellIcon name="FolderGit2" size={16} />}
                onActivate={() => shell.notifications.show({ level: "info", title: "Project switcher opened" })}
              />
            </Menu.Item>
          </Menu.Content>
        </Menu.Positioner>
...
const TicketsWidget = (props: { input: ShellWidgetRenderInput }) => {
  const { input } = props;
      ...
      <ScrollArea flex="1" minH="0">
        {dashboardTickets.map((ticket) => (
          ...
          >
            ...
            >
              <ShellIcon name="ExternalLink" size={14} />
            </IconButton>
          </HStack>
        ))}
      </ScrollArea>
...
const ExtensionRouteWidget = (props: { input: ShellWidgetRenderInput }) => {
  const { input } = props;
...
const SettingsWidget = (props: { input: ShellWidgetRenderInput }) => {
  const { input } = props;
...
const StatusWidget = (props: { input: ShellWidgetRenderInput }) => {
  const { input } = props;

  return (
    <HStack h="full" gap="md" px="sm" minW="0">
      <HStack gap="xs">
        <ShellIcon name="KanbanSquare" size={13} />
        <Text textStyle="label/XS/regular">{dashboardTickets.length} tickets</Text>
      </HStack>
      <HStack gap="xs">
        <ShellIcon name="Puzzle" size={13} />
        <Text textStyle="label/XS/regular">{input.shell.commands.listCommands().length} commands</Text>
      </HStack>
      <Box flex="1" />
      <Text textStyle="label/XS/regular" color="fg.muted" truncate>
        dashboard-shell story
      </Text>
    </HStack>
  );
};
...
const SessionWidget = () => (
  <ChatPanel
    conversationKey="dashboard-shell-session"
