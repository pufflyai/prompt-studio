...
export const ReactRendererWidget = (props: RendererWidgetProps) => {
  const { input } = props;
  const rendererIds = input.shell.renderers.listRenderers().map((renderer) => renderer.id);
    ...
    <ScrollArea h="full" minH="0" contentProps={{ p: "md" }}>
      <Stack gap="md" w="full">
        <Grid templateColumns="repeat(auto-fit, minmax(10rem, 1fr))" gap="sm">
          ...
          <RendererPanel title="Shell state">
            <Stack gap="xs">
              <RendererRow
                icon="PanelsTopLeft"
                label="Registered widgets"
                value={String(input.shell.layout.listWidgets().length)}
              />
        ...
        <HStack gap="sm">
          <Button
            size="sm"
            variant="subtle"
            onClick={() => input.shell.layout.openWidget(bridgeWidgetId, { resource: bridgeResource })}
          >
            <ShellIcon name="Cable" />
            Bridge renderer
          </Button>
        </HStack>
      </Stack>
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data.action === "notify") {
        input.shell.notifications.show({
          level: "info",
      ...
      void input.shell.commands.executeCommand(openReactCommandId).then(input.refresh);
    };
