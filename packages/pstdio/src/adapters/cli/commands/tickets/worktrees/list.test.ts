...
describe("tickets worktrees list", () => {
  test("lists active worktrees linked to a ticket", async () => {
    const log = mock();
    ...
    expect(table).not.toContain("PS-1_A2");
  ...
  test("outputs JSON when --json flag is set", async () => {
    ...
    expect(output[0].worktree_path).toBe("/repo/.pstdio/workspaces/PS-1_A1");
  });

  test("prints empty state when ticket has no active worktrees", async () => {
    const log = mock();
    const handler = createHandler({
      cwd: () => "/repo",
      resolveProjectId: () => ({ projectId: "proj-1", root: "/repo" }),
    ...
    expect(log).toHaveBeenCalledWith("No worktrees found for ticket PS-1");
  });
});
