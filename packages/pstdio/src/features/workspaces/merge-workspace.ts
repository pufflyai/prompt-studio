...
import { git as defaultGit, mergeWorktree as defaultMergeWorktree } from "pstdio-wt";
...
const toMergePayload = (
  ...
  return {
    ...
    repoPath: repoRoot,
    worktreePath:
      typeof workspace.provider_ref_json?.data.worktreeRoot === "string"
        ? workspace.provider_ref_json.data.worktreeRoot
        : undefined,
    branch,
    workspace: {
      id: workspace.id,
...
export const mergeWorkspace = async (input: MergeWorkspaceInput, deps: Deps = defaultDeps) => {
  const { projectId, workspaceShorthand, deleteAfter } = input;
