...
describe("mergeWorkspace", () => {
  test("squash-merges workspace", async () => {
    ...
    await mergeWorkspace({ projectId: "proj-1", workspaceShorthand: "PS-1_A1" }, { ...baseDeps, squashMerge, log });
  ...
  test("fires git merged event after squash merge", async () => {
    ...
    await mergeWorkspace({ projectId: "proj-1", workspaceShorthand: "PS-1_A1" }, { ...baseDeps, fireGitMerged });
    ...
    expect(fireGitMerged).toHaveBeenCalledWith("proj-1", {
      ...
      workspace: {
        ...
        branch: "workspace/PS-1_A1",
        root_path: `${homedir()}/.pstdio/workspaces/PS-1_A1`,
        anchors_json: [],
  ...
  test("deletes workspace when deleteAfter is true", async () => {
    ...
    await mergeWorkspace(
      { projectId: "proj-1", workspaceShorthand: "PS-1_A1", deleteAfter: true },
      { ...baseDeps, deleteWorkspace, log },
    );
  ...
  test("does not fail or skip cleanup when git merged event dispatch fails", async () => {
    ...
    expect(calls).toEqual([
      "log:Merging changes across the entire repository: /repo",
      "squashMerge",
      "deleteWorkspace",
  ...
  test("refuses to merge a root checkout", async () => {
    await expect(
      mergeWorkspace(
        { projectId: "proj-1", workspaceShorthand: "default" },
        {
          ...baseDeps,
          getWorkspace: async () =>
            makeWorkspace({
