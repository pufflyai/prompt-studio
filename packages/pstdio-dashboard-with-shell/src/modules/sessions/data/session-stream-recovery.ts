...
export const resolveRecoveredStreamMessages = (
  ...
  if (!hydratedMessages) return currentMessages;
  if (hydratedMessages.length < currentMessages.length) return currentMessages;
  if (JSON.stringify(hydratedMessages) === JSON.stringify(currentMessages)) return currentMessages;
