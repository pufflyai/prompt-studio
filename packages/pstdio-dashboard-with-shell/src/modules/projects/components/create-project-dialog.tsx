...
import { FolderPickerDialog } from "../../file-system/components/folder-picker-dialog";
...
export const hasProjectBasicsErrors = (input: ProjectBasicsInput) =>
  input.name.trim().length === 0 || input.repositories.length === 0;
...
export const toggleAgentSelection = (selectedAgentIds: string[], agentId: string) =>
  selectedAgentIds.includes(agentId) ? selectedAgentIds.filter((id) => id !== agentId) : [...selectedAgentIds, agentId];

export const canSubmitAgentSelection = (selectedAgentIds: string[]) => selectedAgentIds.length > 0;
...
export const CreateProjectDialog = (props: CreateProjectDialogProps) => {
  const { open, availableAgents, isSubmitting, onClose, onCreate } = props;
                      ...
                      return (
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={checked}
                            onChange={() => setSelectedAgentIds((current) => toggleAgentSelection(current, agent.id))}
                            disabled={isSubmitting}
                          />
              ...
              <Stack direction="row" gap="1">
                {step === 1 ? (
                  <>
                    ...
                    <Button onClick={handleNext} variant="primary" disabled={isSubmitting}>
                      {t("createProjectDialog.actions.next")}
                    </Button>
                  </>
                ) : (
                  <>
                    <Button onClick={() => setStep(1)} variant="outline" disabled={isSubmitting}>
