...
1. **Create the project** — `POST /projects` inserts the project row and auto-creates default ticket statuses and ticket tags.
...
4. **Side-effect rows are streamed.** Default statuses and tags created during project creation are emitted as `sync:set` events so connected clients stay in sync.
