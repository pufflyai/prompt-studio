...
2. **Register repos** (optional) — `POST /projects/{id}/repos` accepts a name and an existing directory path. It returns `400` if the path is missing or is not a directory. It reuses the repository by path or creates one, then links it through `project_repos`. Required bootstrap, default extension setup, and default workspace creation must succeed before repository and link sync events are published. Failed setup removes only newly created repository rows and links; existing repositories and links remain intact. Default workspace creation is the last required step. Workspace provisioning follows registration and records hook failures as workspace setup errors. If the local `.pstdio/config.json` points to a different active project, the API returns `409`. If it points to a project that no longer exists, the API treats it as stale state and rewrites `.pstdio/config.json` for the new project. If no repos are specified and the command is not run inside a git repo, this step is skipped.
