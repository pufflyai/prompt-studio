...
pstdio tracks conversations between users and AI coding tools as sessions. A session captures the full lifecycle from prompt submission through harness provider execution to completion or failure and bridges the database, harness runtime, API, and dashboard.
...
└─────┬─────┘   └───────┬───────┘
      ...
      └─────────────────┘
         ...
         ▼            ▼            ▼
  ┌────────────┐ ┌──────────┐ ┌──────────────┐
  │  pstdio-db │ │EventStore│ │HarnessRegistry│
  │  sessions  │ │(in-memory│ │              │
  │ workspaces │ │  per-id) │ │              │
  └────────────┘ └──────────┘ └──────┬───────┘
                                     │
...
- `session.harness_session_id` — the external provider's own session/thread ID for providers such as OpenCode or Claude Code.
...
| last_request_started | text          | When last provider request began                       |
| last_request_ended   | text          | When last provider request finished                    |
| harness              | text          | `"claude-code"` or `"opencode"`                        |
| harness_session_id   | text          | External provider session ID (nullable)                |
...
| Status           | Meaning                                     |
| ---------------- | ------------------------------------------- |
| `in_progress`    | Harness provider is actively executing                 |
| `awaiting_input` | Harness provider is waiting for user approval or input |
| `completed`      | Harness provider finished successfully                 |
| `failed`         | Harness provider crashed or returned non-zero exit     |
...
4. Resolve harness provider from request or global default.
5. Create in-memory event store for streaming.
6. Call the provider `start(...)` handler with prompt/title/model and cwd.
7. Persist `harness_session_id` when the provider session starts.
...
Creates ticket, workspace (`Attempt N`), and session in one operation, then starts the harness provider.

- `session.title` is derived from ticket content title.
- `cwd` is repository root if `repo_id` is provided.
- Non-zero provider process exit marks session `failed`.

### 3) Ticket attempt — `POST /v1/tickets/:ticket_id/attempts`

Creates a ticket attempt workspace + session and starts the harness provider.
...
The route always creates a ticket-linked workspace row, emits sync updates for `workspaces` + `ticket_workspaces`, and defaults `start_session` to `true`. When a session is started, it links the session to the workspace, emits `sessions` + `workspaces` updates, and starts the harness provider in the resolved cwd.
...
4. **Same harness provider:** require `harness_session_id`, call the provider `send(...)` or resume handler with `messageOffset` from cached message count.
5. **Different harness provider:** update `session.harness`, clear previous `harness_session_id`, and start a new provider session.
...
1. Try the provider message reader with `harness_session_id` and cwd.
2. If successful: return normalized messages, persist to `session.content`, mark `harness_session_status = connected`.
3. If provider fetch fails: fallback to cached `session.content.messages`, mark `harness_session_status = disconnected`.
4. If no `harness_session_id`: status is `not_connected`.
...
- `approval_request` — provider requests tool permission (`{ id, toolName, toolInput, toolUseId }`)
...
2. **The harness provider is the message authority.** pstdio always tries to fetch messages from the provider first and only falls back to cached content.
...
5. **Follow-ups can switch harness providers.** When the provider changes, the previous `harness_session_id` is cleared and a new session is started with the new provider.

## Current gaps

- Event stores are lost on API restart — no persistence layer. Stale `in_progress` sessions are reattached when the harness provider supports it (OpenCode) or transitioned to `disconnected` otherwise, via the startup sweep (`runStartupTasks` → `resolveOrphanedSessions`; see [Session Status Lifecycle](/architecture/session-status-lifecycle)).
