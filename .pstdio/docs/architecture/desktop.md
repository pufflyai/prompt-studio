...
The ephemeral browser session deliberately discards credentials and browser storage between launches. Desktop keeps the selected project, each project's last resource, saved kanban views, and ordered open project-tab IDs in Electron's user-data directory. Saved kanban snapshots include names, filters, layout settings, default designation, and active selection. The dashboard hydrates host storage before rendering and supplies it through `KanbanRendererStorageProvider`; the UI store remains the snapshot owner. The typed preload accepts only the kanban storage namespace for these writes and cannot write arbitrary workbench keys. Layout data, session selection, and unsent chat drafts stay in the ephemeral browser session and do not survive application quit. A normal browser continues to use `localStorage`. Runtime-served dashboard metadata also forces API and sync requests to remain same-origin, even when a source build supplied a different `VITE_API_BASE_URL`.
