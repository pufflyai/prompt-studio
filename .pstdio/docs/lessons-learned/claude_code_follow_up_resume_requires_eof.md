...
`pstdio` transitions an active session to `completed` or `failed` when the spawned provider process exits.
...
The API session tracker now applies a timeout while waiting for provider process exit. If a provider process hangs anyway, `pstdio` kills it and marks the session as `failed` instead of leaving it stuck forever.
