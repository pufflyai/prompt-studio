...
Implementation automation (for example automatically starting work) is configured with extension commands and extension events, not with status metadata. Ticket status behavior belongs to the planner extension boundary.
