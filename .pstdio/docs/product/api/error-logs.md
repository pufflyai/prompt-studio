...
Prompt Studio emits structured runtime logs (API, extension commands, CLI mutating commands, and lifecycle events) through a shared `pino` logger to a configurable JSONL target.
...
3. Extension command invocation and outcome events must be included in the shared log stream.
