...
Extensions are installed into the user extension root, defaulting to `~/.pstdio/extensions/<install-name>/`, and are enabled per project. One installed source can be enabled in multiple projects with project-specific settings and storage.
...
| Dashboard host | Prompt Studio dashboard | Resolves slots into menus, navigation, views, settings panels, and renderers. |
...
The implemented dashboard UI model uses host-owned slots. Extensions import slot refs such as `projectSlots.headerPrimary`, `ticketSlots.headerOverflow`, or `workspaceSlots.sidebar`, then attach menus, navigation, views, settings panels, and renderers to those slots.

Slots describe the dashboard surface. Optional `when` expressions restrict visibility by command source, active resource type, or active resource metadata.

## Lifecycle

1. A package is installed into the user extension root.
...
6. The dashboard requests extension UI metadata and resolves slot contributions into host UI.
