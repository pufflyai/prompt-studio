...
This repository runs several layers of tests, and not all of them exercise real harness provider binaries.
...
```bash
PSTDIO_HARNESSES=${E2E_HARNESSES:-fake}
```

That means:

- Plain `bun run validate` uses the `fake` harness provider for UI e2e.
- Default UI session creation paths spawn fake sessions only.
- Default UI coverage does not start Claude Code sessions.

## Harness Flags

Use `E2E_HARNESSES` to opt into a different harness provider for Playwright UI tests.
...
```bash
E2E_HARNESSES=fake bun run --cwd packages/e2e test:ui
```
...
```bash
E2E_HARNESSES=claude-code bun run --cwd packages/e2e test:ui
```
...
Those specs configure the `claude-code` harness provider and do start real Claude sessions.

If Claude is not on your `PATH`, provide a custom binary:

```bash
E2E_HARNESSES=claude-code E2E_CLAUDE_BINARY=/absolute/path/to/claude bun run --cwd packages/e2e test:ui
```

### OpenCode

```bash
E2E_HARNESSES=opencode bun run --cwd packages/e2e test:ui
```
...
Even when tests do not start a real Claude session, some non-e2e tests may still probe local harness provider availability.

Examples:

- Claude availability checks can call `claude --version`.
- OpenCode availability checks can call `opencode --version`.
- CLI harness listing can call `which claude` or `which opencode`.

Those checks verify installation state only. They do not create or resume harness sessions by themselves.

## Practical Rule

If you run `bun run validate` with no extra environment variables, the test suite should not start Claude Code sessions.

If you set `E2E_HARNESSES=claude-code`, you are explicitly opting into real Claude-backed Playwright session tests.
