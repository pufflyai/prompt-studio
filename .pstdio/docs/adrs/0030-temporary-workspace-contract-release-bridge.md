...
Alpha.10 root workspaces can provision every linked repository. During their trusted `workspace.provision` and `workspace.ready` events, the bridge resolves working files and processes from the existing repository context by registered ID. It rechecks project ownership and never redirects a recorded worktree or remote target. The default workspace and `projectFiles` still use the first linked folder. This temporary lifecycle projection has one resolver and is removed with repository linking at the alpha.11 cutover.
