...
export const DocsOutline = (props: DocsOutlineProps) => {
  ...
  useEffect(() => {
    setActiveHeadingId(extractHeadings(markdown)[0]?.id ?? "");
  }, [markdown]);

  useEffect(() => {
    const nextMatch = matchHeadingElements(extractHeadings(markdown), env.getDocument());
    const observer = new (env.getWindow().IntersectionObserver)(
      (entries) => {
  ...
  const handleHeadingSelect = (headingId: string) => {
    ...
    headingElementsMatchRef.current.elementsByHeadingId
      .get(headingId)
