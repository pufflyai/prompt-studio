...
describe("native title bar appearance", () => {
  test("uses the rendered title bar colors for native controls", () => {
    expect(titleBarOverlayOptions({ color: "rgb(249, 251, 250)", symbolColor: "rgb(20, 24, 22)" })).toEqual({
  ...
  test("accepts dark and custom theme colors", () => {
    expect(titleBarOverlayOptions({ color: "rgb(14, 16, 22)", symbolColor: "rgb(247, 248, 248)" }).color).toBe(
      "rgb(14, 16, 22)",
    );
  });
